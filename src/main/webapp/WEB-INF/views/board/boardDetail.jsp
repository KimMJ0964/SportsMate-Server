...
<link
	href="${pageContext.request.contextPath}/resources/css/board/boardDetail.css?after"
	rel="stylesheet">
<title>게시글 상세</title>
<style>
</style>
	...
	<div class="wrap">
			...
			<div class="bd-meta-info">
				<c:choose>
					<c:when test="${not empty board.modifyDate}">
						${board.memName } / ${board.modifyDate }(수정됨) / ${board.view }
					</c:when>
					<c:otherwise>
		...
		<div class="bd-content-container">
			...
			<div class="bd-button-container">
				<div class="bd-button">
					<a href="${downloadLink}" style="color: white;" download>파일
						다운로드</a>
				</div>

				<c:if
					test="${loginMember != null && loginMember.memNo == board.memNo}">
					<button class="bd-button"
						onclick="location.href = 'modifyMove.bd?mpage=${board.boardNo}'">수정하기</button>
				</c:if>
				<button class="bd-red-button" data-bs-toggle="modal"
					data-bs-target="#reportModal"
					onclick="setReportData(${board.boardNo}, 0, ${board.memNo })">신고하기</button>
				<div>
					<img class="bd-like"
						src="${pageContext.request.contextPath}/resources/images/board_like.png"
						onclick="location.href = 'boardLike.bd?bno=${board.boardNo }'" />
		...
		<div class="bd-comment-container">
			<form method="post" action="writeReply.bd?bno=${board.boardNo}">
				<div class="bd-comment-section">
					<textarea class="bd-comment-textarea" rows="3"
						style="resize: none; width: 100%;" name="content" id="content"></textarea>
					<button type="submit" class="bd-button" style="float: right;">댓글
						작성 완료</button>
				</div>
			</form>
		</div>
		...
		<div class="bd-comment">
			...
			<div class="bd-reply-form">
				<textarea rows="3"
					style="width: 100%; height: 120px; border: 1px solid #307DFA; resize: none; padding: 10px; font-size: 16px; box-sizing: border-box; border-radius: 8px;"></textarea>
				...
				<c:if test="${comments.comParentNo == 0}">
					<div class="bd-one-comment-container">
						<div class="bd-comment-info">
							<c:choose>
								<c:when test="${not empty comments.changeName}">
									<img class="bd-comment-profile-img"
										src="${pageContext.request.contextPath}/resources/images/userProFile/${comments.changeName}" />
								</c:when>
								<c:otherwise>
									<img class="bd-comment-profile-img"
						...
						<div class="bd-button-container">
							...
							<button class="bd-button" data-bs-toggle="modal"
								data-bs-target="#commentModal"
								onclick="setCommentData(${comments.comNo}, ${board.boardNo})">답글
								작성</button>
						</div>
							...
							<c:forEach var="reply" items="${comment}">
								...
								<c:if test="${reply.comParentNo == comments.comNo}">
									<div class="bd-reply">
										<c:choose>
											<c:when test="${not empty reply.changeName}">
												<img class="bd-comment-profile-img"
													src="${pageContext.request.contextPath}/resources/images/userProFile/${reply.changeName}" />
											</c:when>
											<c:otherwise>
												<img class="bd-comment-profile-img"
													src="${pageContext.request.contextPath}/resources/images/user_default_profile.png" />
									...
									<div class="bd-button-container">
										...
										<button class="bd-button" data-bs-toggle="modal"
											data-bs-target="#commentModal"
											onclick="setCommentData(${reply.comParentNo}, ${board.boardNo})">답글
											작성</button>
									</div>
								</c:if>
							</c:forEach>
					...
					<form method="post" action="boardReport.bd">
						<div class="modal-body">
							...
							<div class="bd-report-content">
								<textarea class="bd-report-content-textarea" id="report-content"
									name="pnContent" class="report-content"
						...
						<div class="modal-footer">
							...
							<input type="hidden" id="report-comNo" name="comNo"> <input
								type="hidden" id="report-reporterNo" name="reporterNo">

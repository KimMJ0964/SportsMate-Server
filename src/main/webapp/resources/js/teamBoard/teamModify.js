...
document.addEventListener("DOMContentLoaded", () => {
    // data-region 속성에서 값 가져오기
    const selectElement = document.getElementById("activityArea");
    const activityArea = selectElement.dataset.region; // data-region 값 가져오기

    // 선택값 설정
    selectElement.value = activityArea;
});
...
const setMaxMember = ()=>{
    ...
    const selectElement = document.getElementById("teamMaxPerson");
    const maxPerson = selectElement.dataset.person; // data-region 값 가져오기

    // 선택값 설정
    selectElement.value = maxPerson;
}
